...
namespace Utility.Toolkit
{
    ...
    public class InternationalCountryCodes
    {
        public List<CountryTelCode> TelCodes { get; set; }

        public InternationalCountryCodes()
        {
            InitTelCodes();
        }

        public CountryTelCode GetCountryCode(string mobile)
        {
            CountryTelCode cc = null;
            if (string.IsNullOrWhiteSpace(mobile))
            {
                return null;
            }

            for (int i = 0; i < TelCodes.Count; i++)
            {
                var p = TelCodes[i];
                if (mobile.Contains(p.Pfx))
                {
                    cc = p;
                    break;
                }
